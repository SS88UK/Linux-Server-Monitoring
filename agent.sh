...
if [ -e /etc/lsm-ss88/data.log ]
then
	data=($(cat /etc/lsm-ss88/data.log))
...
# System load cache
echo "$time $cpu $io $idle $rx $tx" > /etc/lsm-ss88/data.log
...
then
	timeout -s SIGKILL 30 wget -q -o /dev/null -O /etc/lsm-ss88/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://lsm.ss88.us/api"
else
	wget -q -o /dev/null -O /etc/lsm-ss88/agent.log -T 25 --post-data "$data_post" --no-check-certificate "https://lsm.ss88.us/api"
	wget_pid=$! 
	wget_counter=0
	wget_timeout=30
